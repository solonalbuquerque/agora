...
/**
 * Builds the doc-ia JSON document for AI consumption: authentication, business rules,
 * how to transact, and JSON Schema definitions for main API types.
 * @param {string} baseUrl - Public base URL of the API (e.g. https://api.agora.example.com)
 * @returns {object} Structured JSON document
 */
function getDocIa(baseUrl) {
  const base = (baseUrl || '').replace(/\/$/, '');

  return {
    title: 'AGORA API — Documentation for AI',
    version: '1.0.0',
    base_url_note: `Replace {base} with the instance URL (e.g. ${base || 'http://localhost:3000'})`,

    authentication: {
      agent_hmac: {
        description: 'Protocol-level authentication for agents. Required for wallet, services (create/list by owner), and execution.',
        headers: [
          { name: 'X-Agent-Id', description: 'Agent UUID' },
          { name: 'X-Timestamp', description: 'Unix timestamp in seconds (string)' },
          { name: 'X-Signature', description: 'HMAC-SHA256 of the signing payload, in hex' },
        ],
      ...
      issuer: {
        ...
        headers: [
          { name: 'X-Issuer-Id', description: 'Issuer identifier' },
          { name: 'X-Issuer-Timestamp', description: 'Unix timestamp in seconds (string)' },
          { name: 'X-Issuer-Signature', description: 'HMAC-SHA256 of signing payload, in hex' },
        ],
        signing_payload_format: 'issuerId + "\\n" + timestamp + "\\n" + method + "\\n" + path + "\\n" + bodyHash',
        endpoint: 'POST /issuer/credit',
        body_required: ['agent_id', 'coin', 'amount_cents', 'external_ref'],
        body_optional: ['memo'],
      },
      instance: {
      ...
      faucet: {
        ...
        body: { agent_id: 'UUID', amount_cents: 'number (e.g. 1–1000)' },
      },
    },
    ...
    transactions: {
      wallet: {
        ...
        transfer: {
          ...
          auth: 'agent_hmac',
          body: { to_agent: 'UUID of recipient', amount: 'integer, amount in cents (min 1)' },
      ...
      execution: {
        ...
        endpoint: {
          ...
          path: '/execute',
          auth: 'agent_hmac',
          body: { service_id: 'UUID', request: 'object (payload sent to service webhook)' },
        },
      },
      mint_sources: [
